# DATAVIZ
A website that allows for interactive visualizations

## Installation
Unzip the file and click on: DATAVIZ.html

## Usage
- An Upload button allows for uploading files for it to be visualized
- A Play button refreshes the visualisation
- The left frames contains a node visualisation
- The right frame contains a matrix visualisation.

## Important Files
- DATAVIZ.html (Actual website incorporates bundled.html nodes visualisation and matrix.html Matrix visualisation)
- Force-Directed.html (Node visualisation that needs to be added into website)
